...
func (a *Account) GetGithubUpdates(r *http.Request) {
	...
	if err != nil {
		session.AddFlash("Error getting user info: " + err.Error())
		return
	}
	...
	resp, err = t.Client().Do(req)
	if err != nil {
		session.AddFlash("Error getting timeline: " + err.Error())
		return
	}
	if resp.StatusCode == 200 {
		defer resp.Body.Close()
		...
		for i := 0; i < len(timeline); i++ {
			...
			profileUrl := timeline[i]["actor"].(map[string]interface {})["url"].(string)
			link := timeline[i]["repo"].(map[string]interface {})["url"].(string)
			var title string
			var text string
